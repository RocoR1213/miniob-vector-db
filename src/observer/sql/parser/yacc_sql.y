...
FunctionExpr *create_function_expression(FunctionExpr::Type function_type,
                                         ...
                                         YYLTYPE *llocp)
{
  ...
  if (arguments != nullptr) {
    children.swap(*arguments);
    delete arguments;
  }

  FunctionExpr *expr = new FunctionExpr(function_type, children);
  expr->set_name(token_name(sql_string, llocp));
  return expr;
}
...
attr_def:
    ...
    | ID type
    {
      ...
      $$->type = (AttrType)$2;
      $$->name = $1;
      $$->length = $$->type == AttrType::VECTORS ? VECTOR_DEFAULT_DIMENSION : 4;
    }
    ;
number:
    NUMBER {$$ = $1;}
...
value:
    ...
    |STRING_TO_VECTOR LBRACE SSS RBRACE {
      ...
      $$ = new Value();
      RC rc = DataType::type_instance(AttrType::VECTORS)->set_value_from_str(*$$, tmp);
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to parse vector literal. value=%s, rc=%s", tmp, strrc(rc));
      }
      free(tmp);
    }
    ;
...
function_expression:
    VECTOR_TO_STRING LBRACE expression_list RBRACE {
      $$ = create_function_expression(FunctionExpr::Type::VECTOR_TO_STRING, $3, sql_string, &@$);
    }
    | DISTANCE LBRACE expression_list RBRACE {
      $$ = create_function_expression(FunctionExpr::Type::DISTANCE, $3, sql_string, &@$);
    }
    ;
