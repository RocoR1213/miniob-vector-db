...
{
  const vector<unique_ptr<Expression>> &children = expression.children();
  switch (expression.function_type()) {
    case FunctionExpr::Type::VECTOR_TO_STRING: {
      if (children.size() != 1) {
        LOG_WARN("VECTOR_TO_STRING expects 1 argument, got %d", children.size());
        return RC::INVALID_ARGUMENT;
      }
      if (children[0]->value_type() != AttrType::VECTORS) {
        LOG_WARN("VECTOR_TO_STRING expects vector argument, got %s", attr_type_to_string(children[0]->value_type()));
    ...
    case FunctionExpr::Type::DISTANCE: {
      if (children.size() != 3) {
        LOG_WARN("DISTANCE expects 3 arguments, got %d", children.size());
        return RC::INVALID_ARGUMENT;
      }

      if (children[0]->type() != ExprType::FIELD || children[0]->value_type() != AttrType::VECTORS) {
        LOG_WARN("DISTANCE first argument must be a vector field");
        return RC::SCHEMA_FIELD_TYPE_MISMATCH;
      }
      if (children[1]->value_type() != AttrType::VECTORS) {
        LOG_WARN("DISTANCE second argument must be a vector");
        return RC::SCHEMA_FIELD_TYPE_MISMATCH;
      }

      int left_len  = children[0]->value_length();
      int right_len = children[1]->value_length();
      if (left_len > 0 && right_len > 0 && left_len != right_len) {
        LOG_WARN("DISTANCE vector dimension mismatch. left_len=%d, right_len=%d", left_len, right_len);
        return RC::SCHEMA_FIELD_TYPE_MISMATCH;
      }

      if (children[2]->type() != ExprType::VALUE || children[2]->value_type() != AttrType::CHARS) {
        LOG_WARN("DISTANCE third argument must be a quoted string constant");
        return RC::INVALID_ARGUMENT;
      }

      const ValueExpr *method_expr = static_cast<const ValueExpr *>(children[2].get());
      FunctionExpr::DistanceMethod method;
      RC rc = FunctionExpr::distance_method_from_string(method_expr->get_value().get_string(), method);
      if (rc != RC::SUCCESS) {
        return rc;
      }
    } break;
    ...
    default: {
      LOG_WARN("unsupported function type. type=%d", static_cast<int>(expression.function_type()));
      return RC::INVALID_ARGUMENT;
    }
  }
