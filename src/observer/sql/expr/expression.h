...
public:
  ...
  static RC distance_method_from_string(const string &method_name, DistanceMethod &method);

private:
  RC calc_value(const vector<Value> &arguments, Value &value) const;
  RC calc_vector_to_string(const vector<Value> &arguments, Value &value) const;
  RC calc_distance(const vector<Value> &arguments, Value &value) const;
