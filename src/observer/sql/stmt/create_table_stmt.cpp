...
RC CreateTableStmt::create(Db *db, const CreateTableSqlNode &create_table, Stmt *&stmt)
{
  ...
  vector<AttrInfoSqlNode> attr_infos = create_table.attr_infos;
  for (AttrInfoSqlNode &attr_info : attr_infos) {
    if (attr_info.type != AttrType::VECTORS) {
      continue;
    }

    const size_t dimension = attr_info.length;
    if (dimension == 0 || dimension > VECTOR_MAX_DIMENSION) {
      LOG_WARN("invalid vector dimension. field=%s, dimension=%d", attr_info.name.c_str(), static_cast<int>(dimension));
      return RC::INVALID_ARGUMENT;
    }
    attr_info.length = dimension * VECTOR_ELEMENT_SIZE;
  }

  stmt = new CreateTableStmt(create_table.relation_name, attr_infos, create_table.primary_keys, storage_format);
