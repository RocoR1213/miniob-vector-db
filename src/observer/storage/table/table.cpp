  ...
  for (int i = 0; i < value_num && OB_SUCC(rc); i++) {
    ...
    if (field->type() == AttrType::VECTORS) {
      if (value.attr_type() != AttrType::VECTORS || value.length() != field->len()) {
        LOG_WARN("invalid vector value. table name:%s,field name:%s,value type:%s,value len:%d,field len:%d",
            table_meta_.name(), field->name(), attr_type_to_string(value.attr_type()), value.length(), field->len());
    ...
    if (value.attr_type() == AttrType::VECTORS) {
      LOG_WARN("cannot cast vector value to non-vector field. table name:%s,field name:%s",
...
RC Table::set_value_to_record(char *record_data, const Value &value, const FieldMeta *field)
{
  ...
  if (field->type() == AttrType::CHARS) {
    ...
    LOG_WARN("vector value length mismatch. field=%s, field len=%d, value len=%d",
...
// A4 创建向量索引
RC Table::create_vector_index(Trx *trx, const FieldMeta *field_meta, const char *index_name, int lists, int probes)
{
  return engine_->create_vector_index(trx, field_meta, index_name, lists, probes);
}
