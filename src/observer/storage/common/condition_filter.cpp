...
RC DefaultConditionFilter::init(Table &table, const ConditionSqlNode &condition)
{
  ...
  if (type_left == AttrType::VECTORS) {
    const int left_len  = left.is_attr ? left.attr_length : condition.left_value.length();
    const int right_len = right.is_attr ? right.attr_length : condition.right_value.length();
    if (condition.comp != EQUAL_TO || left_len != right_len) {
      LOG_WARN("invalid vector comparison. comp=%d, left_len=%d, right_len=%d", condition.comp, left_len, right_len);
      return RC::SCHEMA_FIELD_TYPE_MISMATCH;
    }
  }
